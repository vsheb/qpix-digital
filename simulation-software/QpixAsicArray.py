...
class QpixAsicArray():
    ...
    def _Command(self, timeEnd, command=None):
        ...
        # add the initial broadcast to the queue
        print(f'Line 148: {self._queue.Length()}')
        steps = 0
        self._queue.AddQueueItem(self[0][0], 3, QPByte(self._tickNow, [], None, None), self._timeNow, command=command)
        ...
        while(self._timeNow < timeEnd):
            ...
            while(self._queue.Length() > 0):
                ...
                steps += 1
                nextItem = self._queue.PopQueue()
                print(f'the daq interrogation arrived at asic ({nextItem.asic.row}, {nextItem.asic.col}) at time {nextItem.inTime}')
                if nextItem.asic.row == None and nextItem.asic.col == None:
                    nextItem.asic.isDaqNode = True

                self.ProcessArray(nextItem.inTime)
                print(f'the in time for the interrog is {nextItem.inTime} vs the current time {self._timeNow}')

                # ASIC to receive data
                newProcessItems = nextItem.asic.ReceiveData(nextItem)
                if newProcessItems:
                    print(f'asic ({nextItem.asic.row}, {nextItem.asic.col}) received an interrogation from ({nextItem.QPByte.originRow}, {nextItem.QPByte.originCol})')
                    for item in newProcessItems:
                        self._queue.AddQueueItem(*item)
                    print(f'After asic receives data, queue length is {self._queue.Length()}')

                # if steps == 10:
                #     exit
                if steps%10000 == 0:
                    print(f'the steps = {steps}')

                if self._queue.Length() > 5000:
                    exit()
            self._timeNow += self._deltaT
            self._tickNow += self._deltaTick
            print(f'the time is now {self._timeNow}')
    ...
    def ProcessArray(self, nextTime):
        ...
        while somethingToDo:
            somethingToDo = False
            for asic in self:
                # print(f'In the ProcessArray, processing asic ({asic.row}, {asic.col})')
                newProcessItems = asic.Process(nextTime)
                if newProcessItems:
                    print(f'its new process items are {newProcessItems} from ({newProcessItems[0][-2].originRow}, {newProcessItems[0][-2].originCol})')
                    processed += 1
                    somethingToDo = True
                    for item in newProcessItems:
                        self._queue.AddQueueItem(*item)
        print(f'after array is processed there are {self._queue.Length()} queue items')
        return processed
